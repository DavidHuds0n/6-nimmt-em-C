...
void exibirMesa(Fila** mesa, Lista **maosJogadores, Lista **colecaoCartasPuxadas, Lista* cartasTurno){
    ...
    // Exibe as filas que estão na mesa
    for(int i = 0 ; i < 4 ; i++){
        printf("[FILA %d]\n", i+1);
        exibirFila(mesa[i]);
        printf("\n");
    ...
    // Exibe a coleção do jogador
    printf("[SUA COLEÇÃO]\n");
    if(!exibirLista(colecaoCartasPuxadas[0])){
        printf("(Você não puxou nenhuma carta)\n");
    }
    printf("\n-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n\n");
}
...
int inserirMesa(int qtd_jogadores, Fila **mesa, Lista **colecaoCartasPuxadas, Lista *cartasTurno){
    Carta guardarUltima;
    Carta guardarCarta;
...
int jogar(int qtd_jogadores, Pilha *baralho, Lista **maosJogadores, Fila **mesa, Lista **colecaoCartasPuxadas){
    ...
    for (int i = 0; i < MAX_ROUNDS; i++){ // Número de rodadas até acabar a mão de todos os jogadores
        ...
        if (!inserirMesa(qtd_jogadores, mesa, colecaoCartasPuxadas, cartasTurno)) return 0;
